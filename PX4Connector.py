...
class PX4Connector:
    ...
    async def set_hold_mode(self):
        try:
            ...
            self.logger.warning(f'[{mavlink_timestamp}] PERSON DETECTED: Sending Hold mode MAVLink signal to PX4')
    ...
    async def set_land_mode(self):
        try:
            mavlink_timestamp = time.time()
            self.logger.warning(f'[{mavlink_timestamp}] PERSON DETECTED: Sending Land mode MAVLink signal to PX4')
